<?php defined('RAXANPDI')||exit(); ?>

<!DOCTYPE html>

<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
        <title>SPURR - Directory Employee Application</title>
        <link href="raxan/ui/css/master.css" type="text/css" rel="stylesheet" />
        <!--[if lt IE 8]><link rel="stylesheet" href="raxan/ui/css/master.ie.css" type="text/css"><![endif]-->
        <link href="raxan/ui/css/default/theme.css" type="text/css" rel="stylesheet" />
        <style type="text/css">
            .page-header ul  { margin:0; list-style: none; }
            .page-header ul li {
                float: left;
                margin:0 10px 0 0;
            }
            .page-header ul li a { color:#fff;font-size: 1.12em; padding:2px 5px }
            .page-header ul li a:hover { background-color: #2aa; text-decoration: none}
        </style>
    </head>

    <body>
        <div class="rax-content-pal container round">
            <div class="page-header container rax-header-pal rax-glass pad">
                <h1 class="bmm">SPURR - Directory Application</h1>
                <div class="container rax-toolbar-pal round">
                    <ul>
                        <li><a href="index.php">Home</a></li>
                        <li><a href="employeeList.php">Employee List</a></li>
                        <li><a href="addForm.php">Add Employee</a></li>
                        <!-- <li><a href="#">About</a></li> -->
                    </ul>
                </div>
            </div>
            <div class="page-content master-content prepend1 append1"></div>
            <hr class="space clear" />
        </div>
        <hr class="space clear" />
    </body>

</html>